...
- Der Process Trail verwendet einen Fokusmodus: Nur der aktive Step zeigt Beschreibung und Evidence; abgeschlossene Steps kollabieren zu kompakten, manuell wieder oeffenbaren Zeilen. Ein begrenzter interner Scrollbereich folgt dem aktiven Step, ohne den Seiten-Scroll zu uebernehmen.
- Die UI behandelt Provider-Latenz als Product-Choreografie statt als Debug-Log: Fruehe Schritte erhalten etwa 1,2 Sekunden Mindestpraesenz; der echte `calling_ai`-Status erscheint als `Building your roast` und rotiert belegbare Activity-Texte aus Commit-, Repository- und Dateizaehlern. Diese Texte sind keine zusaetzlichen Backend-Phasen und verzoegern niemals den Result-Reveal.
