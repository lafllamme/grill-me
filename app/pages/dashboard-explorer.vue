    ...
    <div class="mx-auto px-5 pb-24 pt-6 max-w-[1440px] sm:px-8 lg:px-12">
      ...
      <Transition name="dashboard-variant" mode="out-in">
        <main v-if="activeVariant === 'profile'" id="profile-panel" key="profile" class="grid gap-4 lg:grid-cols-12" role="tabpanel" aria-labelledby="profile-tab">
          ...
          <article class="rounded-[28px] bg-surface-container p-6 sm:p-8 lg:col-span-5 lg:p-10"><p class="text-[10px] text-primary-strong tracking-[0.18em] font-meta uppercase">Profile dimensions / radar chart</p><BklitRadarChart class="mt-8" :data="fixture.radarProfile.data" :metrics="fixture.radarProfile.metrics" /><div class="space-y-3 border-t-[1px] border-divider pt-5 mt-6"><div v-for="item in fixture.profile" :key="item.label" class="flex gap-3 items-center"><span class="text-xs text-on-background flex-1">{{ item.label }}</span><span class="text-xs text-on-surface-variant font-meta">{{ item.value }}</span></div></div></article>
