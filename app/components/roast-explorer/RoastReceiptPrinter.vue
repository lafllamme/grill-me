...
<template>
  <section
    class="p-0 border-[1px] border-basalt-200 rounded-[1.5rem] border-solid bg-bone-50 min-h-[52rem] shadow-md relative z-20 overflow-hidden"
  ...
  >
    <div class="rounded-[1.5rem] bg-bone-50 min-h-[48rem] relative overflow-visible">
      <div data-testid="roast-printer-slot" class="rounded-full bg-basalt-950 h-9 shadow-inner inset-x-[10%] top-5 absolute z-40 sm:inset-x-[11%]">
        <div class="mx-auto mt-3 rounded-full bg-surface-container-highest h-1 w-3/4" />
      </div>

      <div class="inset-x-0 bottom-5 top-5 absolute z-10 overflow-hidden sm:bottom-8">
        <div
          class="mx-auto max-w-[34rem] w-[82%] origin-top transition-transform duration-[850ms] ease-out inset-x-0 top-0 absolute z-30 sm:w-[74%] motion-reduce:transition-none"
          :style="{ transform: paperTranslate }"
        >
          <div
            class="bg-bone-200 h-[40rem] w-full shadow-lg relative"
          >
            <div
