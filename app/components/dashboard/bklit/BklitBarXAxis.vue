  ...
  <g>
    <template v-for="(item, index) in context.data" :key="index">
      <template v-if="props.showAllLabels || context.data.length <= props.maxLabels || index === context.hoveredIndex.value || index % Math.max(1, Math.ceil(context.data.length / props.maxLabels)) === 0">
        ...
        <text :x="context.xAt(index)" y="299" text-anchor="middle" :opacity="context.tooltipX.value === null ? 1 : Math.min(1, Math.max(0, (Math.abs(context.xAt(index) - context.tooltipX.value) - props.tickerHalfWidth) / 20))" :class="context.hoveredIndex.value === index ? 'fill-background font-body font-semibold' : 'fill-on-surface-variant font-body'" class="text-[12px]">
