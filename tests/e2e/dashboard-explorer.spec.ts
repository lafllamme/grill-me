...
test.describe('dashboard explorer analysis states', () => {
  ...
  test('previews the loading layout without starting an API request', async ({ page }) => {
    ...
    const profileRadar = page.getByTestId('profile-radar-panel')
    await expect(profileRadar).toHaveCount(1)
    await expect(profileRadar).toHaveCSS('visibility', 'hidden')
    await expect(profileRadar.locator('.legend-container')).toHaveCount(1)
  ...
  test('keeps deterministic scores out of the dashboard until the final stream event', async ({ page }) => {
    ...
    await expect(profileRadar).toHaveCSS('visibility', 'hidden')
    await expect(profileRadar.locator('.legend-container')).toHaveCount(1)
    await profileRadar.evaluate(element => element.setAttribute('data-persistence-marker', 'true'))

    await expect(page.getByTestId('dashboard-analysis-state')).toHaveCount(0)
    await expect(profileRadar).toBeVisible()
    await expect(profileRadar).toHaveAttribute('data-persistence-marker', 'true')
